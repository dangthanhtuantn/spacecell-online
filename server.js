...
const FOOD_COUNT = 800;           // ← đã giảm mạnh
const BOT_COUNT = 20;
const TICK_MS = 33;               // ← 30 FPS, tiết kiệm CPU
const ITEM_MAX = 10;
...
function mkBot(i) {
    return {
        id: 'b' + i, x: rnd(500, GW - 500), y: rnd(500, GH - 500), mass: rnd(15, 80), vx: 0, vy: 0,
        col: BCOLS[i % 15], name: BNAMES[i % 15] + (i >= 15 ? '_' + (i / 15 | 0) : ''),
        atx: rnd(0, GW), aty: rnd(0, GH), at: 0, st: 0, nextAI: 0
    };
}

function mkPlayer(id, name, color, flag, screenW, screenH) {
    return {
        id, name: name || 'Player', color: color || '#00cfff', flag: flag || null,
        ...
        screenW: screenW || 1920, screenH: screenH || 1080
    };
}
...
setInterval(() => {
    ...
    pArr.forEach(p => {
        ...
        const visiblePlayers = pArr.filter(q => q.id !== p.id && q.x > left && q.x < right && q.y > top && q.y < bottom);
        const visibleBots = bots.filter(b => b.x > left && b.x < right && b.y > top && b.y < bottom);
        const visibleBullets = bullets.filter(b => b.x > left && b.x < right && b.y > top && b.y < bottom);
...
server.listen(PORT, () => console.log(`🚀 SpaceCell Optimized v2 running at http://localhost:${PORT}`));